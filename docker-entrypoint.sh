...
# GITLAB_OMNIBUS_CONFIG に設定を追加する関数
append_to_config() {
    if [ -n "$GITLAB_OMNIBUS_CONFIG" ]; then
        export GITLAB_OMNIBUS_CONFIG="$GITLAB_OMNIBUS_CONFIG;"
    fi
    export GITLAB_OMNIBUS_CONFIG="$GITLAB_OMNIBUS_CONFIG$1"
}
...
configure_gitlab() {
  ...
  if [ -n "$DATABASE_URL" ]; then
      ...
      append_to_config "gitlab_rails['db_password'] = '$pass'"
  ...
  if [ -n "$REDIS_PASSWORD" ]; then
    append_to_config "redis['password'] = '$REDIS_PASSWORD'"
